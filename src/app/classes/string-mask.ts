export class StringMask {
    ...
    placeholderCharsPattern: RegExp;
    ...
    constructor(pattern:string, options) {
        ...
        let patternLetters = this.pattern.split("");
        let placeholderChar: string;
        let placeholderCharsPatternStr: string = "";
        while(patternLetters.length > 0) {
            placeholderChar = patternLetters.pop();
            if(!this.tokens[placeholderChar]) placeholderCharsPatternStr += "\\"+placeholderChar;
        }
        if(placeholderCharsPatternStr) this.placeholderCharsPattern = new RegExp("["+placeholderCharsPatternStr+"]", "g");
    }

    public process(value): {result: string, valid: boolean, cleanValue: string} {
        if (!value) {
            return {result: '', valid: false, cleanValue: ''};
        }
        value = value + '';
        let cleanValue: string = value.slice(0).replace(this.placeholderCharsPattern, "");
        value = cleanValue;
        ...
        function continueCondition(options) {
            if (!inRecursiveMode && !recursive.length && self.hasMoreTokens(pattern2, patternPos, steps.inc)) {
                // continue in the normal iteration
                return true;
            } else if (!inRecursiveMode && recursive.length &&
                self.hasMoreRecursiveTokens(pattern2, patternPos, steps.inc)) {
            ...
            if (inRecursiveMode) {
                ...
                if (options.reverse && valuePos >= 0) {
                    patternPos++;
                    pattern2 = self.insertChar(pattern2, pc, patternPos);
                    return true;
        ...
        return {result: formatted, valid: valid, cleanValue: cleanValue};
    };

    public apply = function(value) {
        return this.process(value).result;
    };

    public validate = function(value) {
        return this.process(value).valid;
    };

    public cleanValue(value) {
        return this.process(value).cleanValue;
    }
