...
@Component({
  selector: 'app-pool-invitations',
  templateUrl: './pool-invitations.component.html',
  styleUrls: ['./pool-invitations.component.scss'],
  providers: [
    EntityService,
    CrmMemberService,
    ReservationService,
...
export class PoolInvitationsComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.reservationsSubscription = this.reservationsService.queryParamSubject.subscribe(params => {
      let query = this.reservationsService.fromEntity('ReReservation')
                                                    ...
                                                    .andRaw("Type eq cast('1', Nirvana.Shared.Enums.ReservationNewType)")
    ...
    this.reservationCustomersService.queryParamSubject.subscribe(
      params => {
        ...
        let query = this.reservationCustomersService.fromEntity('ReReservationCustomer')
                                                    ...
                                                    .andRaw("Reservation/Type eq cast('1', Nirvana.Shared.Enums.ReservationNewType)")
                                                    .take(params['pageSize'])
                                                    .page(params['page']);
  ...
  openCustomerSeatInfoBox(event, customer) {
    ...
    component.instance.actionsAllowed = false;
  ...
  private convertToGroupSale() {
    this.notificationService.add({ type: 'danger', text: 'not implemented' });
  }
  ...
  private prepareCsvModel(customers, seats) {
    ...
    seats.forEach(seat => {
      ...
      let modelSeat = {
        ...
        section: seat.VenueSeat.VenueRow.VenueBlock.Section.Name || '',
        block: seat.VenueSeat.VenueRow.VenueBlock.Name || '',
        row: seat.VenueSeat.VenueRow.Name || seat.VenueSeat.VenueRow.RowNumber || '',
        seat: seat.VenueSeat.Name || seat.VenueSeat.SeatNo || '',
      }
  ...
  goToCreate() {
    this.router.navigate(['create/rsvp-pool-invitation'], {relativeTo: this.route});;
  }
}
