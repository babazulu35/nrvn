...
@Injectable()
export class ProductProductService extends  BaseDataService {
  count : BehaviorSubject<number> =  new BehaviorSubject(0);
    data : BehaviorSubject<ProductProduct[]> =  new BehaviorSubject([]);
    ...
    constructor(http : Http, storeService : StoreService, authenticationService:AuthenticationService){
      super(http, 'PProductProduct', storeService, authenticationService);
    }
    getRawData() : ProductProduct[]{
      return this.storeService.getData('PProductProduct');
    }
    getData() : BehaviorSubject<ProductProduct[]>{
      return this.data;
    }
    create(event:Object){
