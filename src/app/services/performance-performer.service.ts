...
@Injectable()
export class PerformancePerformerService extends  BaseDataService {
  count : BehaviorSubject<number> =  new BehaviorSubject(0);
	data : BehaviorSubject<PerformancePerformer[]> =  new BehaviorSubject([]);
	...
	constructor(http : Http, storeService : StoreService, authenticationService:AuthenticationService){
		super(http, 'EPerformancePerformer', storeService, authenticationService);
	}
	getRawData() : PerformancePerformer[]{
		return this.storeService.getData('EPerformancePerformer');
	}
	getData() : BehaviorSubject<PerformancePerformer[]>{
		return this.data;
	}
	create(event:Object){
