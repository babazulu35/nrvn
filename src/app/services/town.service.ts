...
@Injectable()
export class TownService extends  BaseDataService{
	count : BehaviorSubject<number> =  new BehaviorSubject(0);
	data : BehaviorSubject<Town[]> =  new BehaviorSubject([]);
	...
	constructor(http : Http, storeService : StoreService, authenticationService : AuthenticationService){
		super(http, 'LTown', storeService, authenticationService);
	}
	getRawData() : Town[]{
		return this.storeService.getData('LTown');
	}
	getData() : BehaviorSubject<Town[]>{
		return this.data;
	}
	create(town){
