...
@Injectable()
export class VenueSeatService extends  BaseDataService {
  count : BehaviorSubject<number> =  new BehaviorSubject(0);
  data : BehaviorSubject<VenueSeat[]> =  new BehaviorSubject([]);
  ...
  constructor(http : Http, storeService : StoreService, authenticationService:AuthenticationService){
    super(http, 'EVenueSeat', storeService, authenticationService);
  }
  getRawData() : VenueSeat[]{
    return this.storeService.getData('EVenueSeat');
  }
  getData() : BehaviorSubject<VenueSeat[]>{
    return this.data;
  }
  create(event:Object){
