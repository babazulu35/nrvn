...
@Injectable()
export class AttributeService extends BaseDataService{
	count : BehaviorSubject<number> =  new BehaviorSubject(0);
	data : BehaviorSubject<Attribute[]> =  new BehaviorSubject([]);
	...
	constructor(http : Http, storeService : StoreService, authenticationService : AuthenticationService){
		super(http, 'AAttribute', storeService, authenticationService);
	}
	getRawData() : Attribute[]{
		return this.storeService.getData('AAttribute');
	}
	getData() : BehaviorSubject<Attribute[]>{
		return this.data;
	}
	create(town){
