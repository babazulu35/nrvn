...
@Injectable()
export class AttributeTypeService extends BaseDataService{
	count : BehaviorSubject<number> =  new BehaviorSubject(0);
	data : BehaviorSubject<AttributeType[]> =  new BehaviorSubject([]);
	...
	constructor(http : Http, storeService : StoreService, authenticationService : AuthenticationService){
		super(http, 'AAttributeType', storeService, authenticationService);
	}
	getRawData() : AttributeType[]{
		return this.storeService.getData('AAttributeType');
	}
	getData() : BehaviorSubject<AttributeType[]>{
		return this.data;
	}
	create(town){
